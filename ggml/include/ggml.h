...
#define GGML_N_TASKS_MAX (-1)
    ...
    // Stamp a stable identifier on the graph. ggml-cuda's CUDA-graph cache
    // uses this for the fast-path early-return: when cgraph->uid != 0 and
    // matches the cached graph's uid, the per-call node-property memcmp
    // walk is skipped (see ggml_cuda_graph_update_required). Useful when
    // a caller knows it is reusing an identical persistent cgraph across
